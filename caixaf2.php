...
if($conta == 0){
	header("Location:nada2.php");
}
while($linha = mysqli_fetch_array($pesquisa)){

  $nome = $linha['nome'];
  $_SESSION['id'] = $id = $linha['id'];
  $valor = $linha['valor'];
 $_SESSION['quantidade'] = $quantidade = $linha['quantidade'];
}
...
<form method="post" name="fomr" action="recebe2.php" class="forprin">
